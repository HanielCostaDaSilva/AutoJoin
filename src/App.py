...
#Posiveis situações
situacao_lista =["ALTERADO","ADICIONADO","DUPLICADA"]

#Percorremos o dataframe dos funcionarios atualizados
for func_index, pk in func_atualizado_df.get_columns(pk_column):
    
    func_atualizado = func_atualizado_df[func_index]
    ...
    func_index_list = func_df.get_index_by_colum(pk_column, pk) 
    ...
    if len(func_index_list) > 0:        
        for index in func_index_list:
            ...
            for coluna in func_df.columns:
                ...
                if func_df.get_column(index , coluna) == "":
                    func_df.alter_column_index(index, coluna, func_atualizado[coluna].upper())
                    alterado=True
             
            if alterado:  
                    func_df.alter_column_index(index, situacao_col, situacao_lista[0])
        ...
        func_atualiz_index_duplic = func_atualizado_df.get_index_by_colum(pk_column, pk)

        if len(func_atualiz_index_duplic) > 1:
            
            #Adicione o valor no dataframe a partir da primeira duplicada (o segundo valor da lista)
            for func_dup_index in func_atualiz_index_duplic[1:]:
                #Mude a situação da duplicada para ALTERADA
                
                func_atualizado_df.alter_column_index(func_dup_index, situacao_col, situacao_lista[2])
                
                func_df.add_row(**func_atualizado_df[func_dup_index])        
                
    #Caso não encontre o funcionario no dataframe, adicione-o no dataframe original
    else:
        func_atualizado_df.alter_column_index(index,situacao_col,situacao_lista[2]) 
        func_df.add_row(**func_atualizado_df[func_index])
...
#Laranja se for UMA COLUNA duplicada 
orange = PatternFill(start_color="DE9940", end_color="DE9940", fill_type="solid")
